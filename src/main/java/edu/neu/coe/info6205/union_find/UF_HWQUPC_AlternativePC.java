 ...
 */
public class UF_HWQUPC_AlternativePC implements UF {
    /**
     ...
     */
    public UF_HWQUPC_AlternativePC(int n, boolean pathCompression) {
        count = n;
        parent = new int[n];
        height = new int[n];
        for (int i = 0; i < n; i++) {
     ...
     * @throws IllegalArgumentException if {@code n < 0}
     */
    public UF_HWQUPC_AlternativePC(int n) {
        this(n, true);
    }
    ...
    public int find(int p) {
        ...
        if (pathCompression) {
            ...
            while (p != root) {
                //getting root and assigning to temp variable
                int temp = parent[p];
                //assigning p's root to it's grand parent
                parent[p] = parent[parent[p]];
                //updating p to root value until the while loop exits and sets the parent[] of all the components to it's root
                p = temp;
            }
        }
        return root;
    ...
    public static void BenchmarkWQUPC(int n) {
        UF_HWQUPC_AlternativePC ufAlt = new UF_HWQUPC_AlternativePC(n);
        UF_HWQUPC uf = new UF_HWQUPC(n);
        Random rd = new Random();
        Benchmark<Boolean> bm = new Benchmark_Timer<>("Weighted Quick Union with Single pass path compression for " + n + " sites", b -> {
            for (int i = 0; i < n - 1; i++) {
        ...
        double benchmarkWQUPC = bm.run(true, 200);
        System.out.println(benchmarkWQUPC + " milliseconds");
        Benchmark<Boolean> bmAlt = new Benchmark_Timer<>("Alternate Weighted Quick Union with Two pass path compression for " + n + " sites", b -> {
            for (int i = 0; i < n - 1; i++) {
        ...
        double benchmarkWQUPCAlt = bmAlt.run(true, 200);
